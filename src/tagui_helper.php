...
$helper_flow_context = "";
$helper_flow_action = "";
$helper_flow_filename = "";
$helper_flow_options = "";
$helper_flow_browser = "";
$helper_flow_found = false;

// remove filler words that have limited impact on meaning of intention
$raw_intent = str_replace(" is ", " ", $raw_intent);
$raw_intent = str_replace(" are ", " ", $raw_intent);
$raw_intent = str_replace(" was ", " ", $raw_intent);
$raw_intent = str_replace(" were ", " ", $raw_intent);
$raw_intent = str_replace(" my ", " ", $raw_intent);
$raw_intent = str_replace(" me ", " ", $raw_intent);
$raw_intent = trim($raw_intent);
...
if ((strpos($raw_intent, "using chrome") !== false) and
    ...
    $helper_flow_browser = " chrome";
    $raw_intent = substr($raw_intent, 0, strrpos($raw_intent, "using chrome"));
...
$token = explode(" ", $raw_intent);
$helper_flow_action = $token[0]; // split into tokens separated by space
for ($count = 1; $count < sizeof($token); $count++) {
    $helper_flow_context = "";
    $helper_flow_options = "";
...
// echo "./tagui " . $helper_flow_filename . " quiet " . trim($helper_flow_options . $helper_flow_browser) . "\n";
    if (file_exists($helper_flow_filename)) {
        $helper_flow_found = true;
        break;
    }
}

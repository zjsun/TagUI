...
$source_csv = $argv[1];
if ($source_csv == "") die("ERROR - specify csv filename as first parameter\n");
...
fclose($source_file);
$csv_count = @count((array)$csv_data);

// do some prep, transpose datatable and output csv file
$csv_result = array();
$target_csv = str_replace("_transpose.csv", ".csv", $source_csv);
...
foreach ($csv_result as $csv_line) {
    fputcsv($target_file, $csv_line);
}
fclose($target_file);

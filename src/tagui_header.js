...
var p8 = casper.cli.raw.get(7); // p9 (${10}) is less portable
...
var excel_json = {};
var excel_files = [];
var excel_focus = false;
var excel_visible = true;
var excel_password = '';
...
function save_text(file_name, info_text) {
    var ds;
    if (flow_path.indexOf('/') !== -1) ds = '/'; else ds = '\\';
    if (!file_name) {
        save_text_count++;
...
function save_table(file_name, selector) {
    return save_table(file_name, selector, false);
}

// for saving table from website to file
function save_table(file_name, selector, append) {
    var ds;
    if (flow_path.indexOf('/') !== -1) ds = '/'; else ds = '\\';
    if (!file_name) {
        save_text_count++;
...
function excel_range_to_size(input_range) { // for converting excel range to array size
    ...
    var range_start = input_range.split(':')[0].trim();
    var range_end = input_range.split(':')[1].trim();
    var row_start = range_start.match(/\d+/g);
    var row_end = range_end.match(/\d+/g);
    var col_start = range_start.match(/[a-zA-Z]+/g);
    var col_end = range_end.match(/[a-zA-Z]+/g);
// use special number 1234567 to mark column and row selection, since excel max rows is 1048576 and max cols is 16384
    ...
    col_start = col_start.toString().toUpperCase();
    var col_start_count = 0;
    var col_start_len = col_start.length;
    ...
    col_end = col_end.toString().toUpperCase();
    var col_end_count = 0;
    var col_end_len = col_end.length;
    for (ce_pos = 0; ce_pos < col_end_len; ce_pos++) {
        col_end_count += (col_end.charCodeAt(ce_pos) - 64) * Math.pow(26, col_end_len - ce_pos - 1);
    }
    var width = col_end_count - col_start_count + 1;
    return [width, height];
}

function read_excel(input_excel) { // for reading from excel target
    if (user_system == 'windows') {
        var workbook_file = input_excel.split(']')[0].slice(1).trim();
        input_excel = input_excel.split(']')[1];
        var sheet_name = input_excel.split('!')[0].trim();
        var cell_range = input_excel.split('!')[1].trim();
        workbook_file = abs_file(workbook_file);
        if (excel_files.indexOf(workbook_file) == -1) excel_files.push(workbook_file);
        var fs = require('fs');
        if (!fs.exists(workbook_file))
            casper.echo('ERROR - cannot find Excel file ' + workbook_file).exit();
        var excel_password_code = '';
        if (excel_password != '')
            excel_password_code = ',,,,"' + excel_password.replace(/\"/g, '\\"') + '"';
        var excel_visible_code = 'objExcel.Visible = True';
        if (!excel_visible) excel_visible_code = 'objExcel.Visible = False';
        var excel_focus_code = '';
        if (excel_focus) excel_focus_code = '' +
        ...
        casper.waitForExec('cscript excel_steps.vbs //NoLogo', null, function (response) {
            ...
            var range_size = excel_range_to_size(cell_range);
            if (range_size[0] > 1 || range_size[1] > 1) {
                excel_result += ' ';
                excel_result = excel_result.split('[EXCEL_DELIMITER]');
                var excel_array = [];
...
// special handling with number 1234567 for column and row selection
                if (range_size[0] == 1234567) {
                    range_size[0] = excel_result.pop();
                    range_size[1] = excel_result.pop();
                } else if (range_size[1] == 1234567) {
                    range_size[0] = excel_result.pop();
                    range_size[1] = excel_result.pop();
                } else {
                    excel_result.pop();
                    excel_result.pop();
                } // remove row and column count data from excel_result
                for (row = 0; row < range_size[1]; row++) {
                    excel_array.push(excel_result.splice(0, range_size[0]));
                }
            ...
            this.echo('ERROR - Excel automation exceeded ' + (3 * casper.options.waitTimeout / 1000).toFixed(1) + 's timeout').exit();
        }, (3 * casper.options.waitTimeout));
        ...
        var column_range = cell_range;
        if (column_range.indexOf(':') == -1) column_range = cell_range + ':' + cell_range;
        ...
        var excel_focus_code = '';
        if (excel_focus) excel_focus_code = 'activate\r\n\t';
...
function size_to_excel_range(cell, width, height) { // for converting array to range
    var row_start = parseInt(cell.match(/\d+/g));
    var row_end = row_start + height - 1;
    var col_start = cell.match(/[a-zA-Z]+/g)[0].toUpperCase();
    ...
    if (col_end.length == 1) {
        ...
        mid_char = '-';
        last_char = col_end;
    } else if (col_end.length == 2) {
        first_char = '-';
        mid_char = col_end[0];
        last_char = col_end[1];
    } else {
        first_char = col_end[0];
        mid_char = col_end[1];
        last_char = col_end[2];
    }
    for (count = 0; count < width - 1; count++) {
        last_char = String.fromCharCode(last_char.charCodeAt() + 1);
        if (last_char != '[') continue; else last_char = 'A';
        if (mid_char == '-') {
            mid_char = 'A';
            continue;
        } else {
            mid_char = String.fromCharCode(mid_char.charCodeAt() + 1);
            if (mid_char != '[') continue; else mid_char = 'A';
            if (first_char == '-') {
                first_char = 'A';
                continue;
    ...
    col_end = first_char + mid_char + last_char;
    col_end = col_end.replace(/-/g, '');
    return cell + ':' + col_end + row_end.toString();
}

function write_excel(output_excel) { // for writing to excel target
    if (user_system == 'windows') {
        var workbook_file = output_excel.split(']')[0].slice(1).trim();
        output_excel = output_excel.split(']')[1];
        var sheet_name = output_excel.split('!')[0].trim();
        var cell_range = output_excel.split('!')[1].trim();
...
function excel_retrieve(right_param) { // check OS before starting Excel automation
    ...
    if (right_param.match(/\[.*\.(xl.|xl..|xml|csv)\].*![A-Z0-9]*/i) == null) {
        excel_result = '';
        if (isNaN(right_param)) excel_result = eval(right_param);
...
function read_word(input_word) { // for reading from word target
    if (user_system == 'windows') {
        ...
        word_file = abs_file(word_file);
        var fs = require('fs');
        if (!fs.exists(word_file))
        ...
        word_steps += 'WScript.Echo objHTML.ParentWindow.ClipboardData.GetData("text")';
        save_text('word_steps.vbs', word_steps);
        ...
        casper.waitForExec('word_steps.cmd', null, function (response) {
            word_result = '';
            word_result = require('fs').read('word_steps.out').trim();
        }, function () {
            this.echo('ERROR - Word automation exceeded ' + (3 * casper.options.waitTimeout / 1000).toFixed(1) + 's timeout').exit();
        }, (3 * casper.options.waitTimeout));
    } else if (user_system == 'mac') {
        var word_file = input_word.split(']')[0].slice(1).trim();
        word_file = abs_file(word_file);
        ...
        word_steps += 'do shell script "pbpaste > word_steps.out 2>&1"';
        save_text('word_steps.scpt', word_steps);
...
function read_pdf(input_pdf) { // for reading from pdf target
    if (user_system == 'windows') {
        ...
        pdf_steps += 'WScript.Echo objHTML.ParentWindow.ClipboardData.GetData("text")';
        save_text('pdf_steps.vbs', pdf_steps);
        ...
        casper.waitForExec('pdf_steps.cmd', null, function (response) {
            pdf_result = '';
            pdf_result = require('fs').read('pdf_steps.out').trim();
        }, function () {
            this.echo('ERROR - PDF automation exceeded ' + (3 * casper.options.waitTimeout / 1000).toFixed(1) + 's timeout').exit();
        }, (3 * casper.options.waitTimeout));
    } else if (user_system == 'mac') {
        var pdf_file = input_pdf.split(']')[0].slice(1).trim();
        pdf_file = abs_file(pdf_file);
        ...
        pdf_steps += 'do shell script "pbpaste > pdf_steps.out 2>&1"';
        save_text('pdf_steps.scpt', pdf_steps);
...
function exist(element_identifier) {
    ...
    while (Date.now() < exist_timeout) {
        if (present(element_identifier)) return true; else sleep(100);
    }
    ;
    return false;
}
...
// if tx() returns xps666('/html') means that the element is not found, so set element_visible to false
        if (element_located.toString() == xps666('/html').toString()) element_visible = false;
...
function appeared(element_identifier) {
    ...
    while (Date.now() < appeared_timeout) {
        if (visible(element_identifier)) return true; else sleep(100);
    }
    ;
    return false;
}
...
// if tx() returns xps666('/html') means that the element is not found, so set element_count to 0
    if (element_located.toString() == xps666('/html').toString()) element_count = 0;
...
function get_text(source_text, left_marker, right_marker, optional_count) {
    ...
    var left_position = source_text.indexOf(left_marker);
    if (left_position == -1) return '';
    var right_position = source_text.indexOf(right_marker, left_position + 1);
    if (right_position == -1) return '';
    if (optional_count > 1) {
        var occurrence_count = 2;
        while (occurrence_count <= optional_count) {
            occurrence_count++;
            left_position = source_text.indexOf(left_marker, right_position + 1);
            if (left_position == -1) return '';
            right_position = source_text.indexOf(right_marker, left_position + 1);
            if (right_position == -1) return '';
        }
    }
...
// chrome methods as casper methods replacement for chrome integration
    chrome.exists = function (selector) { // different handling for xpath and css to support both
        ...
        try {
            ...
            if (ws_json.result.result.value > 0) return true; else return false;
        } catch (e) {
            return false;
        }
    };
...
chrome_step('Runtime.evaluate',{expression: chrome_context+'.querySelector(\''+selector+'\').focus()'});}}; */
    ...
    chrome.scrollIntoViewIfNeeded = function (selector) { // helper function to scroll element into view
        if ((selector.toString().length >= 16) && (selector.toString().substr(0, 16) == 'xpath selector: ')) {
            ...
            chrome_step('Runtime.evaluate', {expression: chrome_context + '.querySelector(\'' + selector + '\').scrollIntoViewIfNeeded()'});
        }
    }
...
// chrome runtime.evaluate is different from casperjs evaluate, do some processing to reduce gap
    ...
    chrome.withFrame = function (frameInfo, then) { // replace casperjs frame for switching frame context
        ...
        casper.then(function _step() { // track frame (x,y) offset so that steps within frame work correctly
            ...
            chrome_step('Runtime.evaluate', {expression: new_context + ' = document.evaluate(\'(//frame|//iframe)[@name="' + frameInfo + '" or @id="' + frameInfo + '"]\',' + chrome_context + ',null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null).snapshotItem(0).contentDocument'});
        ...
        casper.then(function _step() {
            if (chrome_context == 'subframe_context') {
                chrome_step('Runtime.evaluate', {expression: 'subframe_context = null'});
...
// convert 'wait for' step to 'hover' step, to wait until timeout for element to appear and hover on it
    ...
    switch (get_intent(live_line)) {
        ...
        case 'ask':
            ...
            break;
        case 'telegram':
            return telegram_intent(live_line);
            break;
        case 'keyboard':
        ...
        case 'timeout':
            ...
            break;
        case 'excel':
            return excel_intent(live_line);
            break;
        case 'word':
            return word_intent(live_line);
            break;
        case 'pdf':
            return pdf_intent(live_line);
            break;
        case 'code':
...
function escape_quote(script_line) { // helper function for string context intents
    if (script_line == '') return '';
    current_context = 'string'; // default is string
    ...
    for (char_counter = 0; char_counter < script_line.length; char_counter++) {
        if ((script_line.charAt(char_counter) == "'") && (current_context == 'string')) {
            script_line = script_line.substr(0, char_counter) + '\\\'' + script_line.substr(char_counter + 1);
            char_counter++;
        }
...
function file_url(absolute_filename) { // helper function to append file:// according for opening local files
    ...
    if (absolute_filename.substr(1, 1) == ':') return 'file:///' + absolute_filename;
...
function escape_bs(input_string) { // helper function to escape backslash characters and friends
    ...
    input_string = String(input_string); // to handle variable input that is a number
...
function table_intent(raw_intent) {
    ...
    if (params == '') return "this.echo('ERROR - target missing for " + raw_intent + "')";
        ...
        if (check_tx(table_identifier(param1))) {
            var append = param2.indexOf(" with append") >= 0;
            if (append) param2 = param2.substr(0, param2.indexOf(" with append"));
            return "save_table('" + abs_file(param2) + "',tx('" + table_identifier(param1) + "'), " + append + ")";
...
function telegram_intent(raw_intent) {
    raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
    var params = ((raw_intent + ' ').substr(1 + (raw_intent + ' ').indexOf(' '))).trim();
    var param1 = (params.substr(0, params.indexOf(' '))).trim();
    var param2 = (params.substr(1 + params.indexOf(' '))).trim();
...
function excel_intent(raw_intent) {
    ...
    var excel_params = raw_intent.split('=');
    var left_param = excel_params[0].trim();
    var right_param = excel_params[1].trim();
    if (excel_params[2]) right_param += '=' + excel_params[2].trim(); // to handle case of formula assignments eg "=A1"
    left_param = esc_bs(left_param);
    right_param = esc_bs(right_param); // to escape backslash \ in Windows folder paths
    left_param = left_param.replace(/\\\\'/g, "\\'");
    right_param = right_param.replace(/\\\\'/g, "\\'");
...
function word_intent(raw_intent) {
    ...
    var word_params = raw_intent.split('=');
    var left_param = word_params[0].trim();
    var right_param = word_params[1].trim();
    left_param = esc_bs(left_param);
    right_param = esc_bs(right_param); // to escape backslash \ in Windows folder paths
    left_param = left_param.replace(/\\\\'/g, "\\'");
    right_param = right_param.replace(/\\\\'/g, "\\'");
...
function pdf_intent(raw_intent) {
    ...
    var pdf_params = raw_intent.split('=');
    var left_param = pdf_params[0].trim();
    var right_param = pdf_params[1].trim();
    left_param = esc_bs(left_param);
    right_param = esc_bs(right_param); // to escape backslash \ in Windows folder paths
    left_param = left_param.replace(/\\\\'/g, "\\'");
    right_param = right_param.replace(/\\\\'/g, "\\'");
...
casper.selectOptionByValue = function (selector, valueToMatch) { // solution posted in casperjs issue #1390
    this.evaluate(function (selector, valueToMatch) {
        ...
        Array.prototype.forEach.call(select.children, function (opt, i) { // loop through list to select option
            if (!found && ((opt.value == valueToMatch) || (opt.text == valueToMatch))) {
...
casper.countElements = function (selector) { // use casperjs in-built function to maximize compatibility
    ...
    try {
        if (count_result > 0) return count_result; else return 0;
    } catch (e) {
        return 0;
    }
};

...
$script = $argv[1];
if ($script == "") die("ERROR - specify flow filename as first parameter\n");
...
$repo_count = 0;
if (file_exists(getenv('custom_csv_file'))) { // load datatable or legacy datatable / repository
    ...
    if (@count((array)$repo_data[$repo_count]) == 1) $repo_count -= 1;
...
if (file_exists($local_repo_location)) { // load local repository file if it exists for objects and keywords
    ...
    if ($repo_count != 0) {
        $repo_count++;
        fgetcsv($local_repo_file);
    } // +1 if array has data, discard header record
...
if (strpos(strtolower(file_get_contents('tagui_config.txt')), "var tagui_language = 'english';") == false) { // section which includes translation engine for handling flows in other languages
    $temp_argv1 = $argv[1];
    $temp_argv2 = $argv[2];
    $temp_argv3 = $argv[3];
...
if (getenv('tagui_turbo_mode') == 'false') {
    $script_content = file_get_contents($script . '.js');
...
function expand_intent($script_line, $indentation_for_tagui_step)
{ // function to call other scripts for reusability
    if ((strpos(strtolower(trim($script_line)), 'tagui ') === 0) or (strtolower(trim($script_line)) == 'tagui')) {
        ...
        if ($params == "")
            ...
            $indentation_for_tagui_step .= str_replace(ltrim($script_line), '', $script_line);
            $expanded_intent = "";
            ...
            while (!feof($temp_input_file)) {
                $expanded_intent .= expand_intent(fgets($temp_input_file), $indentation_for_tagui_step);
            }
            fclose($temp_input_file);
            return $expanded_intent;
        }
    } else return $indentation_for_tagui_step . rtrim($script_line) . "\n";
}
...
function parse_intent($script_line)
{
    ...
    if (!in_array(get_intent($script_line), array("integration_block", "run", "dom", "js", "r", "py", "vision", "code"))) {
        $script_line = escape_quote($script_line);
    } // to avoid breaking JavaScript compilation with '
...
// below use buffer to handle integration code blocks if inside integration code block
    ...
    if ($intent_type == "integration_block") {
        $GLOBALS['integration_block_body'] .= $indentation_tracker . $script_line . "[END_OF_LINE]";
        return "";
    } else {
...
function escape_quote($script_line)
{ // helper function for string context intents
    ...
    for ($char_counter = 0; $char_counter < strlen($script_line); $char_counter++) {
        if ((substr($script_line, $char_counter, 1) == "'") and ($current_context == "string")) {
            $script_line = substr_replace($script_line, "\'", $char_counter, 1);
            $char_counter++;
        }
...
function esc_bs($script_line)
{ // helper to escape backslash to echo properly
    if ($script_line == "") return "";
    return str_replace("\\", "\\\\", $script_line);
}
...
// replacement code for [END_OF_LINE] custom token to denote line break is done at py, r, vision intents
        ...
        case "r finish":
        {
            ...
            $script_line = str_replace(";; ", "; ", str_replace("[END_OF_LINE]", "; ", $GLOBALS['integration_block_body']));
            break;
        }
        case "vision finish":
        {
...
// check intent of step for interpretation into casperjs code
    switch ($intent_type) {
        ...
        case "ask":
            ...
            break;
        case "telegram":
            return telegram_intent($script_line);
            break;
        case "keyboard":
        ...
        case "timeout":
            ...
            break;
        case "excel":
            return excel_intent($script_line);
            break;
        case "word":
            return word_intent($script_line);
            break;
        case "pdf":
            return pdf_intent($script_line);
            break;
        case "code":
...
{
    if (strpos($raw_intent, "=") === false) return false;
...
// to let if/for/while handle multiple steps/code use the { and } steps to define block
    ...
    if (substr($raw_intent, 0, 2) == "//") {
        $GLOBALS['real_line_number'] = $GLOBALS['real_line_number'] - 0.5;
        return true;
    }
    if (substr($raw_intent, -1) == ";") return true;
    if (substr($raw_intent, 0, 9) == "function ") return true;
// assume = is assignment statement, kinda acceptable as this is checked at the very end
    if (strpos($raw_intent, "=") !== false) return true;
    return false;
}
...
{ // used to be a helper function primarily to end frame_intent by closing parsed step block
    ...
    if ($GLOBALS['inside_while_loop'] == 1) return " // end_fi while loop marker";
    return "";
}
...
{
    if (substr(strtolower($raw_intent), 0, 4) == "www.") $raw_intent = "https://" . $raw_intent;
    $twb = $GLOBALS['tagui_web_browser'];
    ...
    if ($twb == 'chrome') {
        ...
        $chrome_call .= "chrome_step('Page.navigate',{url: '" . $raw_intent . "'}); sleep(1000);\n";
    }
...
function tap_intent($raw_intent)
{
    ...
    if (is_sikuli($params)) {
        ...
        return "casper.then(function() {" . call_sikuli($abs_intent, $abs_params);
...
function type_intent($raw_intent)
{
    ...
    if (is_sikuli($param1) and $param2 != "") {
        ...
        return "casper.then(function() {" . call_sikuli($abs_intent, $abs_param1);
    } // use sikuli visual automation as needed
    if (($param1 == "") or ($param2 == ""))
...
// special handling for [clear] keyword to clear text field by doing an extra step to clear the field
        if (substr($param2, 0, 7) == "[clear]") {
            if (strlen($param2) > 7) $param2 = substr($param2, 7); else $param2 = "";
...
function down_intent($raw_intent)
{
    ...
    $param1 = trim(substr($params, 0, strpos($params, "to ")));
    $param2 = trim(substr($params, 3 + strpos($params, "to ")));
...
function dump_intent($raw_intent)
{
    ...
    $params = trim(substr($raw_intent . " ", 1 + strpos($raw_intent . " ", " ")));
    $param1 = trim(substr($params, 0, strrpos($params, " to ")));
    $param2 = trim(substr($params, 4 + strrpos($params, " to ")));
...
function snap_intent($raw_intent)
{
    ...
    if (is_sikuli($param1) and (strpos($params, " to ") !== false)) {
        ...
        $abs_params = abs_file($params);
        $abs_intent = str_replace($params, $abs_params, $raw_intent);
        return "casper.then(function() {" . call_sikuli($abs_intent . ' to snap_image()', $abs_params);
...
function table_intent($raw_intent)
{
    ...
    if ($params == "") echo "ERROR - " . current_line() . " target missing for " . $raw_intent . "\n";
        ...
        $append = false;
        if (strpos($param2, " with append") !== false){
            $append = true;
            $param2 = trim(substr($param2, 0, strpos($param2, " with append")));
        }
        return "casper.then(function() {" . "{techo('" . $raw_intent . "');" . beg_tx("'+table_identifier('" . $param1 . "')+'") .
            "save_table('" . abs_file($param2) . "',tx(table_identifier('" . $param1 . "'))". "," . ($append ? "true" : "false") .");" . end_tx("'+table_identifier('" . $param1 . "')+'");
...
function run_intent($raw_intent)
{
    ...
    if (strtolower($raw_intent) == "run begin") {
        ...
        return "";
    } else if (strtolower($raw_intent) == "run finish") {
...
function vision_intent($raw_intent)
{
    ...
    if ($params == "") echo "ERROR - " . current_line() . " Sikuli command(s) missing for " . $raw_intent . "\n"; else
        return "casper.then(function() {" . call_sikuli($safe_intent, 'for vision step');
...
{
    $excel_params = explode("=", $raw_intent);
    $left_param = trim($excel_params[0]);
    $right_param = trim($excel_params[1]);
    if (@count((array)$excel_params) > 2)
    ...
    $left_param = str_replace("\\\\'", "\\'", $left_param);
    $right_param = str_replace("\\\\'", "\\'", $right_param);
...
{
    $word_params = explode("=", $raw_intent);
    $left_param = trim($word_params[0]);
    $right_param = trim($word_params[1]);
    $left_param = esc_bs($left_param);
    $right_param = esc_bs($right_param); // to escape backslash \ in Windows folder paths
    $left_param = str_replace("\\\\'", "\\'", $left_param);
    $right_param = str_replace("\\\\'", "\\'", $right_param);
...
{
    $pdf_params = explode("=", $raw_intent);
    $left_param = trim($pdf_params[0]);
    $right_param = trim($pdf_params[1]);
    $left_param = esc_bs($left_param);
    $right_param = esc_bs($right_param); // to escape backslash \ in Windows folder paths
    $left_param = str_replace("\\\\'", "\\'", $left_param);
    $right_param = str_replace("\\\\'", "\\'", $right_param);
...
// take only lines starting with { or } as code blocks for processing, otherwise will break many valid javascript code
    ...
    if (($code_block_intent == "if") or ($code_block_intent == "else if") or ($code_block_intent == "else")
        or ($code_block_intent == "popup") or ($code_block_intent == "frame")) {
        if (substr($logic, 0, 1) == "}")
            $GLOBALS['code_block_tracker'] = substr($GLOBALS['code_block_tracker'], 0, $last_delimiter_pos);
    ...
    } else if ($code_block_intent == "dframe") {
        if (substr($logic, 0, 1) == "}") {
            $code_block_footer = "});";
            $GLOBALS['code_block_tracker'] = substr($GLOBALS['code_block_tracker'], 0, $last_delimiter_pos);
        }
    } else if ($code_block_intent == "while") {
        if (substr($logic, 0, 1) == "}") {

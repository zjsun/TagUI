...
$source_flow = $argv[1];
if ($source_flow == "") die("ERROR - specify flow filename as first parameter\n");
$direction = $argv[2];
if ($direction == "") die("ERROR - specify to or from as second parameter\n");
$language = $argv[3];
if ($language == "") die("ERROR - specify language as third parameter\n");
...
if (file_exists('languages/' . $language . '.csv')) {
    ...
    if ($language_data[$language_count][0] == '') $language_count -= 1;
} // -1 for EOF
...
{
    if ($script_line == "") return ""; // avoid next line character if none
...
// use special padding to reduce mistakes by preventing false replacement when a match happens mid-string
    ...
    for ($language_check = 1; $language_check <= $GLOBALS['language_count']; $language_check++) {
        ...
        if (strpos($GLOBALS['start_keywords'], '|' . $GLOBALS['language_data'][$language_check][0] . '|') !== false) {
            ...
            if ($replace_count > 0) $start_word = $GLOBALS['language_data'][$language_check][0];
                ...
                $script_line = str_replace('[START_OF_LINE]' . $GLOBALS['language_data'][$language_check][$GLOBALS['column_from']] . '[END_OF_LINE]', '[START_OF_LINE]' . $GLOBALS['language_data'][$language_check][$GLOBALS['column_to']] . '[END_OF_LINE]', $script_line, $replace_count);
                if ($replace_count > 0) $start_word = $GLOBALS['language_data'][$language_check][0];
            }
            ...
            if ($start_word == 'check') {
                ...
                $array_script_line[0] = str_replace(' ' . $GLOBALS['language_data'][$language_check][$GLOBALS['column_from']] . ' ', ' ' . $GLOBALS['language_data'][$language_check][$GLOBALS['column_to']] . ' ', $array_script_line[0]);
    ...
    fwrite($translate_log, $script_line . "\n");

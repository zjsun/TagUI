...
if ($service_settings == "")
    ...
    fclose($service_in_file);
    chmod('tagui_service.in', 0600); // change permissions for security
    ...
    fclose($service_log_file);
    chmod('tagui_service.log', 0600); // change permissions for security
}

// get last service count and increment by 1, or return 1 if it is first entry in queue
function get_next_service_count($service_file)
{
    $service_in_content = file_get_contents($service_file);
    $start_pos = strrpos($service_in_content, "[");
    $end_pos = strrpos($service_in_content, "]");
